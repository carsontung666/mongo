/**
 * Correctness for the covered tree-BFS $graphLookup fast path (gate
 * internalQueryEnableGraphLookupIndexScan). Uses the ConDB adjacency shape:
 * startWith/connectFrom = node_id, connectTo = parent_id, restrictSearchWithMatch
 * = {tree_id}, covering index allops_tree_parent_path = (tree_id, parent_id, path,
 * node_id). Gate off vs on must return the same set of descendant node_ids.
 *
 * @tags: [
...
// Diamond-free tree (unique parent, no cross edges), plus a second tree that shares
// node_id/parent_id values so restrictSearchWithMatch on tree_id is load-bearing.
//
//   n0
//   ├── n1
//   │   ├── n3
...
// maxDepth 0 is direct children only — not the rest of the subtree.
