...
try {
    ...
    // limit that lands on a 16MB stash boundary (~80 x 200KB).
    for (const lim of [1, 79, 80, 81, 160]) {
        assertExpress({a: 7, b: 7}, {c: 1}, undefined, lim);
        const limited = coll.find({a: 7, b: 7}).sort({c: 1}).limit(lim).toArray();
        assert.eq(lim, limited.length, `limit ${lim} across an oversized batch lost documents`);
        assert.eq(bigDocs.slice(0, lim).map(d => d._id),
                  limited.map(d => d._id),
                  `limit ${lim} across an oversized batch returned the wrong documents`);
    }
