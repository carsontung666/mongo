...
const IndexCatalogEntry* findEqualityPrefixIndex(OperationContext* opCtx,
    ...
    while (it->more()) {
        ...
        // Tree-only: connectFrom must live in this non-multikey index. No FETCH fallback —
        // a non-covering prefix would mis-handle array connectFrom on textbook graphs.
...
bool CommonMongodProcessInterface::graphLookupTreeBFS(
    ...
    // Covering keys are not collection documents. Only emit a scalar pulled from the index,
    // and only when that field is in the key. Full `as` documents fall back to stock BFS.
    ...
    // Same role as GraphLookUpStage::_visitedFromValues: do not re-expand a connectFrom
    // already used as a parent. Emission is not de-duped here (stock de-dups by _id;
    // each index entry is one document).
    ...
    for (const auto& s : startValues) {
        // Missing startWith must not seek: omitting connectTo from the key bound would
        // scan the whole equality prefix (e.g. every node in the tree). Stock also treats
        // missing/null connectTo differently than a regular $match; refuse and let it run.
        if (s.missing()) {
            return false;
        }
        visited.insert(s);
