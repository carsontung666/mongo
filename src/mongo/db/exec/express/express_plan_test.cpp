...
TEST_F(ExpressPlanTest, TestLookupViaUserIndexWithMatchingQuery) {
    ...
    LookupViaUserIndex<FetchFromCollectionCallback> iterator(
        ExpressKeyOperands{filter.firstElement()},
        indexEntry->getIdent(),
...
TEST_F(ExpressPlanTest, TestLookupViaUserIndexWithCompoundEquality) {
    ...
    CollatorInterface* collator = nullptr;
    // Both fields bound, so the seek lands on exactly one key.
    LookupViaUserIndex<FetchFromCollectionCallback> iterator(
        ExpressKeyOperands{filter["a"], filter["b"]},
        indexEntry->getIdent(),
        std::string{indexName},
        collator,
        nullptr);
...
TEST_F(ExpressPlanTest, TestLookupViaUserIndexWithCompoundEqualityAndOpenTrailingField) {
    // The unbound trailing field is descending, so its open bound must be reversed.
