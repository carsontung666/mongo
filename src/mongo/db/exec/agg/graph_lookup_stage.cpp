...
GetNextResult GraphLookUpStage::doGetNext() {
    ...
    if (_fastPathValues) {
        ...
        if (!_params.keepInputFields && _params.as.getPathLength() == 1) {
            return Document{{_params.as.fullPath(), std::move(asVal)}};
        }
        MutableDocument output(_params.keepInputFields ? *_input : Document());
    ...
    if (tryCoveredBFS) {
        if (auto eqs = parseGraphLookupEqualities(_params.additionalFilter)) {
            ...
            const std::string* scalarField =
                _params.absorbedScalarField ? &*_params.absorbedScalarField : nullptr;
            const std::vector<std::string>* projectFields =
                _params.visitedFieldNames ? &*_params.visitedFieldNames : nullptr;
