...
public:
    void run() {
        ...
        {
            WorkingSet ws;
            auto owned = std::make_unique<CountScan>(_expCtx.get(), coll, makeParams(), &ws);
            CountStage countStage(_expCtx.get(), 0, 0, &ws, owned.release());
            auto* child = static_cast<CountScan*>(countStage.getChildren().front().get());
            WorkingSetID wsid = WorkingSet::INVALID_ID;
 ...
 * what a materializing scan reports.
 */
...
public:
    void run() {
        ...
        WorkingSet ws;
        auto owned = std::make_unique<CountScan>(_expCtx.get(), coll, params, &ws);
        // Skip the first counted document and limit to one, so both are exercised.
        CountStage countStage(_expCtx.get(), 1, 1, &ws, owned.release());
        WorkingSetID wsid = WorkingSet::INVALID_ID;
        ...
        ASSERT_EQUALS(3, childStats->keysExamined);
